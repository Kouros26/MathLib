...
namespace lm
{
	...
	public:
		...
		{
			constexpr float rad = static_cast<float>(Vec4<T>::degreesToRadians(angle));
		...
		static Mat4<T> PerspectiveProjection(float fov, float aspect, float near, float far)
		{
			constexpr float scale = tanf(fov * static_cast<float>(M_PI / 360)) * near;
			constexpr float r = aspect * scale;
			constexpr float l = -r;
			constexpr float t = scale;
			constexpr float b = -t;
